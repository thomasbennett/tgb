	...
	elseif ($save_type == 'options' OR $save_type == 'framework') {
		...
		foreach($options as $option_array){
			...
			if(isset($option_array['id'])) { // Non - Headings...
              ...
              if (is_array($new_value)) implode(',',$options_value);
						  update_option($option_array['id'],$new_value);
	...
	foreach ($options as $value) {
		...
		//Start Heading
		 if ( $value['type'] != "heading" )
		 {
			...
			if ( $value['type'] != "info" ) $output .= '<h3 class="heading">'. $value['name'] .'</h3>'."\n";
      ...
      $output .= '<input type="checkbox" class="checkbox of-input" name="'. $of_key .'[]" id="'. $of_key .'" value="'.$key.'" '. $checked .' /><label for="'. $of_key .'">'. $option .'</label><br />';
		...
		case "images":
			...
			foreach ($value['options'] as $key => $option) 
			 ...
			 $i++;
				...
				$output .= '<img src="'.$option.'" alt="" title="'.$key.'" class="of-radio-img-img '. $selected .'" onClick="document.getElementById(\'of-radio-img-'. $value['id'] . $i.'\').checked = true;" />';
				$output .= '</span>';
